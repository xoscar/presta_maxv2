...
export default class ClientRoute extends React.Component {
  ...
  render() {
    if (!this.state.clients && !this.client) {
      return (<h1>Cargando...</h1>);
    }

    const action = this.state.clients.action;
      ...
      case 'show-client':
        ...
        return (
          <div>
            ...
            { this.state.showNewLoan ? <NewLoan client={this.client} /> : '' }
            { this.state.showNewCharge ? <NewCharge client={this.client} /> : '' }
          </div>
        );
